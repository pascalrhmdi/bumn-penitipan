        ...
        <form action="<?= route_to('App\Controllers\Item::update', $item->item_id) ?>" method="POST" enctype="multipart/form-data">
          ...
          <div class="ms-5 mb-4">
            <div class="form-group mb-2 d-flex flex-column">
              ...
              <select class="form-select <?= session('errors.id_umkm') ? "is-invalid" : null; ?>" id="id_umkm" name="id_umkm" aria-label="Select UMKM" autofocus>
                <option value="" selected>Pilih UMKM</option>
                <?php foreach ($umkms as $umkm) : ?>
                  <option value="<?= $umkm->id; ?>" <?php if (old('id_umkm') ===  $umkm->id || $item->id_umkm === $umkm->id) echo "selected"  ?>><?= $umkm->nama_umkm; ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="form-group mb-2 d-flex flex-column">
              ...
              <input type="text" class="form-control <?= session('errors.nama_pemberi') ? "is-invalid" : null; ?>" id="nama_pemberi" name="nama_pemberi" value="<?= old('nama_pemberi') ?? $item->nama_pemberi; ?>">
          ...
          <div class="ms-5 mb-4">
            ...
            <div class="form-group mb-2 d-flex flex-column">
              ...
              <div class="input-group">
                ...
                <input class="form-control <?= session('errors.expired_at') ? "is-invalid" : null; ?>" id="expired_at" name="expired_at" type="date" value="<?= old('expired_at') ?? $item->expired_at->toDateString(); ?>">
              </div>
